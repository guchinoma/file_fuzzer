...
class file_fuzzer:
    ...
    def file_picker(self):
        ...
        shutil.copy("examples\\%s" % file, "text.%s" % self.ext)
    ...
    def monitor_debugger(self):
        ...
        if self.in_accessv_handler != True:
            ...
            print "[*] The access violation handler is doing its business, plz wait"
            while self.running:
                time.sleep(1)
    ...
    def mutate_file(self):
        ...
        fd = open("text.%s" % self.ext, "rb")
        stream = fd.read()
        fd.close()
  
        test_cases = self.test_cases[random.randint(0, len(self.test_cases)-1)]
  
        stream_length = len(stream)
        rand_offset   = random.randint(0,  stream_length - 1 )
        rand_len      = random.randint(1, 1000)  
        test_cases     = test_cases * rand_len
  
        fuzz_file = stream[0:rand_offset]
        fuzz_file += str(test_cases)
...
def print_usage():
    ...
    print "[*]"
    print "[*] file_fuzzer.py -e <Executable Path> -x <File Extention>"
    print "[*]"
    sys.exit(0)
